...
public class ImageUtils {
	...
	/**
	 * Converts the given image to gray scale using the averaging
	 * method as implemented in {@link ColorUtils#toGrayScale()}
	 * @param image
	 */
	public static void imageToGrayScaleAverage(RGB image[][]) {
		for (int i = 0; i < image.length; i++) {
			for (int j = 0; j < image[i].length; j++) {
				image[i][j] = ColorUtils.toGrayScale(image[i][j], GrayScaleMode.AVERAGE);
			}
		}
	}
	...
	public static void imageToGrayScaleLightness(RGB image[][]) {
		for (int i = 0; i < image.length; i++) {
			for (int j = 0; j < image[i].length; j++) {
				image[i][j] = ColorUtils.toGrayScale(image[i][j], GrayScaleMode.LIGHTNESS);
			}
		}
	}
	...
	public static void imageToGrayScaleLuminosity(RGB image[][]) {
		for (int i = 0; i < image.length; i++) {
			for (int j = 0; j < image[i].length; j++) {
				image[i][j] = ColorUtils.toGrayScale(image[i][j], GrayScaleMode.LUMINOSITY);
			}
		}
	}
	
	/**
	 * Converts the given image to a sepia-toned imaged as implemented 
	 * in {@link ColorUtils#toSepia()}
	 * @param image
	 */
